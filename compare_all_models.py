...
import numpy as np
from train_multitask import MultiTaskGeoModel, SpatialAttention
from train_hybrid_cnn_vit import HybridCNNViT
...
def plot_comprehensive_comparison():
    ...
    if baseline_metrics:
        ax3.plot(range(1, baseline_epochs + 1), baseline_metrics["val_accuracy"], 
                'b-', label='Baseline CNN', linewidth=2)
    if multitask_metrics:
    ...
    if baseline_metrics:
        ...
        accuracies.append(baseline_metrics["val_accuracy"][-1])
    ...
    params_list = [baseline_params/1e6, multitask_params/1e6, hybrid_params/1e6]
    ...
    if multitask_metrics and hybrid_metrics:
        # Compare multi-task models on auxiliary tasks
...
def generate_comparison_table():
    ...
    print("\n📊 PERFORMANCE METRICS")
    print("-" * 80)
    print(f"{'Metric':<30} {'Baseline CNN':<20} {'Multi-Task CNN':<20} {'Hybrid CNN-ViT':<20}")
    print("-" * 80)
    
    # Country Accuracy
    baseline_acc = f"{baseline_metrics['val_accuracy'][-1]:.2f}%" if baseline_metrics else "N/A"
    ...
    print(f"{'Country Accuracy':<30} {baseline_acc:<20} {multitask_acc:<20} {hybrid_acc:<20}")
    ...
    print(f"{'Region Accuracy':<30} {'N/A':<20} {multitask_region:<20} {hybrid_region:<20}")
    ...
    print(f"{'Climate Accuracy':<30} {'N/A':<20} {multitask_climate:<20} {hybrid_climate:<20}")
    ...
    print(f"{'Final Validation Loss':<30} {baseline_loss:<20} {multitask_loss:<20} {hybrid_loss:<20}")
    ...
    print(f"{'Metric':<30} {'Baseline CNN':<20} {'Multi-Task CNN':<20} {'Hybrid CNN-ViT':<20}")
    print("-" * 80)
    print(f"{'Parameters':<30} {f'{baseline_params/1e6:.2f}M':<20} {f'{multitask_params/1e6:.2f}M':<20} {f'{hybrid_params/1e6:.2f}M':<20}")
    print(f"{'Model Size':<30} {f'{baseline_size:.1f} MB':<20} {f'{multitask_size:.1f} MB':<20} {f'{hybrid_size:.1f} MB':<20}")
    
    # Architecture details
    print("\n🔧 ARCHITECTURE DETAILS")
    print("-" * 80)
    print(f"{'Feature':<30} {'Baseline CNN':<20} {'Multi-Task CNN':<20} {'Hybrid CNN-ViT':<20}")
    print("-" * 80)
    print(f"{'Backbone':<30} {'ResNet50':<20} {'ResNet50':<20} {'ResNet50 + ViT':<20}")
    print(f"{'Attention Mechanism':<30} {'None':<20} {'Spatial Attention':<20} {'Multi-Head Self-Attn':<20}")
    print(f"{'Tasks':<30} {'Single (Country)':<20} {'Multi (C+R+Clim)':<20} {'Multi (C+R+Clim)':<20}")
    print(f"{'Global Context':<30} {'Limited':<20} {'Limited':<20} {'Full (ViT)':<20}")
    print(f"{'Transformer Layers':<30} {'0':<20} {'0':<20} {'6':<20}")
    print(f"{'Attention Heads':<30} {'N/A':<20} {'N/A':<20} {'8':<20}")
    
    # Performance improvements
    if baseline_metrics and multitask_metrics:
        baseline_acc_val = baseline_metrics['val_accuracy'][-1]
    ...
    print("\nHybrid CNN-ViT:")
    print("  • Best of both worlds: CNN local features + ViT global context")
    print("  • Transformer captures long-range spatial dependencies")
    print("  • Multi-head attention for rich feature interactions")
    print("  • State-of-the-art architecture")
    print("  • Superior interpretability through attention visualization")
