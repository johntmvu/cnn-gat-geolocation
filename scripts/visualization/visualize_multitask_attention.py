"""
Attention Visualization for Multi-Task CNN Geolocation Model
Generates heatmaps showing which image regions the model focuses on
Uses spatial attention from the multi-task architecture
"""
...
def predict_with_attention(image_path):
    ...
    # Use spatial attention from multi-task model
    with torch.no_grad():
...
def visualize_single_prediction(image_path, save_path=None, true_country=None):
    ...
    if true_country:
        ...
        title = f"Model: MULTI-TASK CNN WITH SPATIAL ATTENTION\n"
...
def visualize_multiple_samples(num_samples=6):
    ...
    plt.suptitle("Model: MULTI-TASK CNN WITH SPATIAL ATTENTION", fontsize=16, fontweight='bold', y=0.9985)
...
if __name__ == "__main__":
    print("="*60)
    print("MULTI-TASK CNN ATTENTION VISUALIZATION")
    print("="*60)
